...
def handle_teleop(task: str, stage: str):
    print(f"Teleoperation started {task, stage}")
    run_teleop(task, stage)
...
def show_task_view(task):
    ...
    with task_view:
        with ui.column().style('width: 500px; margin-right: 100px;'):
            ...
            with ui.column().style('flex:1'):
                for cam in ["wrist_cam_left", "worms_eye_cam"]:
                    ui.image(f'http://localhost:5000/video_feed?cam={cam}').style(
                        'width: 960px; height: 720px; object-fit: cover; margin-bottom: 10px;')
            with ui.column().style('flex:1'):
